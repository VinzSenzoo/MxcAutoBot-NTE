...
function getAxiosConfig(proxy, additionalHeaders = {}) {
  const headers = {
    ...
    'pragma': 'no-cache',
    'priority': 'u=1, i',
    'referer': 'https://community.mwxtoken.ai/loyalty',
...
async function createSignedPayload(privateKey, address, nonce) {
  try {
    ...
    const messageObj = {
      domain: "community.mwxtoken.ai",
      address: address,
      statement: "Sign in to the app. Powered by Snag Solutions.",
      uri: "https://community.mwxtoken.ai",
      version: "1",
      chainId: 42161,
      nonce: nonce,
      issuedAt: issuedAt
    ...
    const fullMessage = `community.mwxtoken.ai wants you to sign in with your Ethereum account:\n` +
      `${address}\n\n` +
...
async function fetchNonce(address, proxy, context, refCode = 'EVTLKO1X') {
  const url = 'https://community.mwxtoken.ai/api/auth/csrf';
...
async function executeLogin(privateKey, address, nonce, proxy, context, cookies) {
  const url = 'https://community.mwxtoken.ai/api/auth/callback/credentials';
...
async function retrieveBalance(address, proxy, context, cookies, webId = '2f2ef3f3-27f1-4e0c-bd1e-891f09c42690', orgId = 'e7e67897-3371-4083-ba70-c553ed93326b') {
  const url = `https://community.mwxtoken.ai/api/loyalty/accounts?limit=100&websiteId=${webId}&organizationId=${orgId}&walletAddress=${address}`;
...
async function executeDailyCheckin(address, proxy, context, cookies) {
  const url = 'https://community.mwxtoken.ai/api/loyalty/rules/43d44c6c-f9aa-4a2e-ae68-6593bec32960/complete';
  const config = getAxiosConfig(proxy, {
    'Content-Type': 'application/json',
    'Content-Length': '2',
    'Cookie': cookies.join('; ')
...
async function processAccount(account, index, total, proxy) {
  ...
  try {
    ...
    let currentCookies = [`referral_code=EVTLKO1X`, ...nonceData.setCookie.map(ck => ck.split('; ')[0])];
